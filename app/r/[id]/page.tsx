    ...
    return (
        <div className="max-w-[1000px] mx-auto flex gap-x-10 mt-4">
            <div className="w-[65%] flex flex-col gap-y-5">
                <h1>Hola dsds La Secccion de POST Q TAL</h1>
            </div>

            <div className="w-[35%]">
                <Card>
                    <div className="bg-muted p-4 font-semibold">Describe Tu Comunidad</div>
                    ...
                    <div className="p-4">
                        <div className="flex items-center gap-x-3">
                            ...
                            />
                            <Link href={`/r/${data?.name}`} className="font-extrabold">
                                <span className="text-primary font-extrabold">URL</span>r/{data?.name}
                            </Link>
                        </div>
                        ...
                        <Cake className="h-5 w-5 text-muted-foreground" />
                           <p className="text-muted-foreground font-mono">Creado : {new Date(data?.createAt as Date).toLocaleDateString('en-us', {
                            weekday: 'long',
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                        ...
                        <Button asChild className="rounded-full w-full">
                            <Link href={user?.id ? `/r/${data?.name}/create` : "/api/auth/login"}>Crea Una Publicacion📣</Link>
                        </Button>
                    </div>
                </Card>
            </div>
